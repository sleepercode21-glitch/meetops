...
create unique index if not exists ux_poll_options_poll_label
  on poll_options(poll_id, label);

drop trigger if exists trg_poll_options_updated_at on poll_options;
...
drop trigger if exists trg_poll_votes_updated_at on poll_votes;
create trigger trg_poll_votes_updated_at
before update on poll_votes
for each row execute function set_updated_at();

-- Ensure a vote cannot reference an option from another poll.
create or replace function assert_poll_vote_option_matches_poll()
returns trigger as $$
begin
  if not exists (
    select 1
    from poll_options po
    where po.option_id = new.option_id
      and po.poll_id = new.poll_id
  ) then
    raise exception 'poll_votes.option_id % does not belong to poll_id %',
      new.option_id,
      new.poll_id
      using errcode = '23514';
  end if;
...
drop trigger if exists trg_poll_votes_option_matches_poll on poll_votes;
create trigger trg_poll_votes_option_matches_poll
before insert or update of poll_id, option_id on poll_votes
for each row execute function assert_poll_vote_option_matches_poll();

-- Ensure sessions.selected_option_id is an option on a poll for the same session.
create or replace function assert_session_selected_option_matches_session()
returns trigger as $$
begin
  ...
  if not exists (
    ...
    where po.option_id = new.selected_option_id
      and p.session_id = new.session_id
  ) then
    raise exception 'sessions.selected_option_id % does not belong to session_id %',
...
drop trigger if exists trg_sessions_selected_option_matches_session on sessions;
create trigger trg_sessions_selected_option_matches_session
before insert or update of selected_option_id, session_id on sessions
for each row execute function assert_session_selected_option_matches_session();
