...
-- 11. Confirm vote rows always point to options on the same poll
select pv.poll_vote_id, pv.poll_id as vote_poll_id, po.poll_id as option_poll_id
from poll_votes pv
join poll_options po on po.option_id = pv.option_id
where pv.poll_id <> po.poll_id;

-- 12. Confirm selected session options belong to that session
select s.session_id, s.selected_option_id
from sessions s
join poll_options po on po.option_id = s.selected_option_id
join polls p on p.poll_id = po.poll_id
where p.session_id <> s.session_id;

-- 13. Dashboard summaries by group
select
  g.group_id,
  g.name,
  count(distinct m.user_id) as member_count,
  count(distinct s.session_id) filter (where s.status = 'scheduled') as scheduled_sessions,
  count(distinct p.poll_id) filter (where p.status = 'active') as active_polls
from groups g
left join members m on m.group_id = g.group_id
left join sessions s on s.group_id = g.group_id
left join polls p on p.session_id = s.session_id
